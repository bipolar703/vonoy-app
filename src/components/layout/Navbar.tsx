...
const navigationItems = [
  ...
  },
  {
    ...
    dropdown: true,
    items: [
      { name: 'Technical Solutions', path: '/solutions/technical' },
...
const Navbar: React.FC = () => {
  ...
  const { direction, isAtTop } = useScroll({ threshold: 10, throttleMs: 80 });
  const [isNavbarVisible, setIsNavbarVisible] = useState(true);
  const lastDirection = React.useRef<'up' | 'down' | null>(null);

  // Always show navbar when mobile menu is open
  const effectiveNavbarVisible = isMenuOpen ? true : isNavbarVisible;
  ...
  useEffect(() => {
    ...
    if (direction.y === 'up' || isAtTop) {
      setIsNavbarVisible(true);
    } else if (direction.y === 'down') {
      setIsNavbarVisible(false);
    }
    lastDirection.current = direction.y;
  }, [direction.y, isAtTop]);
    ...
    <>
      <nav
        className={`fixed w-full z-50 transition-all duration-300 font-satoshi ${
          isScrolled ? '' : 'bg-transparent py-4'
        } ${isDarkBackground ? 'text-white' : 'text-gray-800'}`}
        style={{
          fontFamily: 'Satoshi, sans-serif',
          backgroundColor: isScrolled ? '#111b2b' : undefined,
          boxShadow: isScrolled ? '0 4px 20px rgba(0,0,0,0.1)' : undefined,
          backdropFilter: isScrolled ? 'blur(12px)' : undefined,
          WebkitBackdropFilter: isScrolled ? 'blur(12px)' : undefined,
          paddingTop: isScrolled ? '0.5rem' : '1rem',
          paddingBottom: isScrolled ? '0.5rem' : '1rem',
          transform: effectiveNavbarVisible ? 'translateY(0)' : 'translateY(-100%)',
      ...
      {/* --- Mobile Menu Panel rendered as sibling, not child --- */}
      {isMenuOpen && (
        <div
          className={`fixed inset-0 z-[10000] bg-[rgba(12,29,44,0.98)] backdrop-blur-2xl md:hidden overflow-y-auto transition-all duration-300 flex flex-col menu-open`}
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <nav
            className={`mobile-menu-container flex flex-col mt-20 px-4 gap-2 menu-open`}
