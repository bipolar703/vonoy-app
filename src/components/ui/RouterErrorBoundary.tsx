 ...
 *
 * A custom error boundary for React Router that displays route errors with a user-friendly interface.
 * Features:
...
const RouterErrorBoundary: React.FC = () => {
  ...
  const routeError = error as CategorizableError;
  const errorCode = categorizeError(routeError);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-primary p-4 text-center">
      ...
      >
        ...
        >
          ...
          >
            {errorStatus ? `Error ${errorStatus}` : 'Unexpected Error'}
          </motion.h1>
